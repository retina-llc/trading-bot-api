...
if (process.env.DB_SSLMODE === "verify-full") {
  if (fs.existsSync(sslCertPath)) {
    ...
    console.error(
      `pg Client SSL certificate not found at path: ${sslCertPath}`,
    );
    throw new Error(`SSL certificate not found at path: ${sslCertPath}`);
  }
} else {
  console.warn(
    "pg Client SSL is not enabled. It is recommended to use SSL for database connections.",
  );
}
...
export const connectDB = async (): Promise<void> => {
  try {
    ...
    console.error(
      "Database connection error via pg Client:",
...
export async function executeQuery(
  ...
  try {
    ...
    console.error(
      "Error executing query via pg Client:",
      (error as Error).message,
    );
    throw error;
  }
...
export const disconnectDB = async (): Promise<void> => {
  try {
    ...
    console.error(
      "Error disconnecting from database via pg Client:",
      (error as Error).message,
    );
    throw error;
  }
