...
export async function getUserBalance(): Promise<number> {
  ...
  try {
    ...
    if (!balances.wallet || balances.wallet.length === 0) {
      ...
      return 0.0; // Return 0 if wallet is empty
    }
    ...
    if (!usdBalance) {
      ...
      return 0.0; // Return 0 if USD balance is not found
    }
...
export async function monitorAndBuy(symbols: string[]) {
  try {
    ...
    const purchaseQuantity =
      buyAmount / (await getTicker(topSymbol)).data.tickers[0].last_price;
    ...
    purchasePrices[topSymbol] = {
      price: parseFloat(
        (await getTicker(topSymbol)).data.tickers[0].last_price,
      ),
      timestamp: Date.now(),
    };
...
export async function checkPriceAndSell(symbol: string, quantity: number) {
  ...
  try {
    ...
    const purchase: { price: number; timestamp: number } | undefined =
      purchasePrices[symbol];
    if (!purchase) {
