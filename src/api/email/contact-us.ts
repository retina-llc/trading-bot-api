...
export class ContactUsController {
  ...
  async handleContactForm(
    @Body() body: { name: string; email: string; wallet_address: string; message: string }
    ...
    try {
      ...
      const adminEmail = this.configService.get<string>('CONTACT_US_EMAIL') || this.configService.get<string>('EMAIL_USER');
      ...
      console.log('[ContactUs] Contact form email sent successfully to admin:', adminEmail);
      return { message: 'Your message was sent successfully. We will get back to you soon!' };
    } catch (error) {
      console.error('[ContactUs] Error sending contact form email:', (error as Error).message);
      throw new HttpException(
