...
const TOP_GAINERS_API_SECRET = '28793edd474d812cbf388bb2a4c497782c867355d80f71d53b4e972e1adec58d';

// API keys for the most volatile coins
const VOLATILE_COINS_API_KEY = 'fd1a780d1beca5a20d712d17e13b6acd443634a4';
const VOLATILE_COINS_API_SECRET = 'ba57f596383e372ee0ee3e46ec6ad3c9686f2bab0bcc6adce7de7356c3c846ce';
...
const DAILY_GAINERS_FILE = path.join(PERSISTENCE_PATH, 'dailyGainersStore.json');
...
let topGainersStore: { [symbol: string]: { count: number, totalPercentageIncrease: number } } = loadData(TOP_GAINERS_FILE) || {};
let dailyGainersStore: { [symbol: string]: { count: number, totalPercentageIncrease: number } } = loadData(DAILY_GAINERS_FILE) || {};
let pumpedCoins: { [symbol: string]: { percentageIncrease: number } } = loadData(PUMPED_COINS_FILE) || {};
...
async function getTickerData(apiKey: string, apiSecret: string): Promise<TickerData[]> {
...
export async function getTopGainers() {
    const tickers = await getTickerData(TOP_GAINERS_API_KEY, TOP_GAINERS_API_SECRET);
    ...
    const updatedTickers = await getTickerData(TOP_GAINERS_API_KEY, TOP_GAINERS_API_SECRET);
    ...
    const gains = updatedTickers.map((ticker: TickerData) => {
        ...
        const percentageIncrease = ((updatedPrice - initialPrice) / initialPrice) * 100;
...
export async function getMostVolatileCoin() {
    ...
    for (let i = 0; i < fetchCount; i++) {
        const tickers = await getTickerData(VOLATILE_COINS_API_KEY, VOLATILE_COINS_API_SECRET);
    ...
    const volatilityScores = Object.keys(volatilityData).map(symbol => {
        const symbolData = volatilityData[symbol];
        const averageFluctuation = symbolData.totalFluctuation / (symbolData.prices.length - 1);
...
export function getTopTrendingCoins() {
    ...
    const positiveTrendingCoins = trendingCoins.filter(coin => coin.averagePercentageIncrease > 0);

    // Sort by the count of appearances in the top 5, and then by average percentage increase
    positiveTrendingCoins.sort((a, b) => b.count - a.count || b.averagePercentageIncrease - a.averagePercentageIncrease);
