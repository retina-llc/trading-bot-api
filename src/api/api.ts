...
const BITMART_API_SECRET =
  "e9ae31b708f572b4b13ef0d9c325bdd5628f3590fc8693120bf49c297a2a583a";
...
export async function getAIRecommendation(symbol: string): Promise<string> {
  try {
    console.log(
      `Received request for AI recommendation with symbol: ${symbol}`,
    );
...
export async function getKline(symbol: string): Promise<any> {
  ...
  try {
    const response = await axios.get(
      `https://api.bitmart.com/v2/klines?symbol=${fullSymbol}&interval=1h`,
    );
